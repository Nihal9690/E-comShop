...
const Home = () => {
  ...
  let searchVal = userStore.searchValue; // Yeh search ka input hai

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 8;

  // ✅ Search Logic: Agar searchValue ho to filtered items dikhao
  const filteredItems = searchVal
  ? allProduct.filter((ele) =>
  ...
  // ✅ Pagination Ko Filtered Items Par Lagaya
  const lastIndex = itemsPerPage * currentPage;
  const firstIndex = lastIndex - itemsPerPage;
  const noOfPages = Math.ceil(filteredItems.length / itemsPerPage);
  const displayedItems = filteredItems.slice(firstIndex, lastIndex);
    ...
    <div className="px-6 sm:px-8 lg:px-12 py-10 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8 max-w-7xl mx-auto">
        {displayedItems.length > 0 ? (
          ...
          <p className="text-center text-gray-600 text-lg col-span-4">No products found.</p>
        )}
      </div>
