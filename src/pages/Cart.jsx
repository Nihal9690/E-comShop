...
const Cart = ({ cartItem, setCartItem }) => {
  ...
  const handleIncrement = (i) => {
    ...
    copyArr[i].price = parseFloat(
      (copyArr[i].price + copyArr[i].price / (copyArr[i].quantity - 1)).toFixed(
        2
      )
    );
    setCartItem(copyArr);
  ...
  const handleDecrement = (i) => {
    ...
    if (copyArr[i].quantity > 1) {
      copyArr[i].price = parseFloat(
        (copyArr[i].price - copyArr[i].price / copyArr[i].quantity).toFixed(2)
      );
      copyArr[i].quantity -= 1;
    } else {
      copyArr.splice(i, 1);
  ...
  return (
    <div className="container mx-auto p-4 lg:p-6 pb-28"> {/* Extra padding at bottom */}
      {cartItem.length > 0 ? (
        <div className="relative bg-white shadow-lg rounded-lg overflow-hidden">
          <div className="max-h-[500px] overflow-auto"> {/* Scrollable content */}
